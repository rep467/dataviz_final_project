...
```{r, fig.alt="This graph is scatterplot graph with each point being a car for the year x and the mpg y. Most data points seem to be below 25mpg with a small cluster after 2010 being at arround 100mpg", warning=FALSE, message=FALSE}
...
## Graph 2
```{r, fig.alt="This graph is scatterplot graph with each point being a car for the year x and the mpg y. This scatterplot shows that the average remained around 19 mpg until 2008. From 2009 it started to increase reaching 23.7 in 2017.", warning=FALSE, message=FALSE}
...
```{r, fig.alt="This graph is an interactive Boxplot graph with one boxplot for each year showing the mpg of all cars. This Boxplot shows that the average remained around 19 mpg until 2008. From 2009 it started to increase reaching 23.7 in 2017. Even in 1984 outliers with mpg above 30 existed.", warning=FALSE, message=FALSE}
...
graph2_reworked <- ggplot(data = fuel_data_filtered_by_fuel_adjusted_mpge_stat_text, aes(x =
  ...
  labs(x = NULL, y = "MPG / MPGe adjusted", title = "MPG of new cars by year", alt = "TEST467")


interactive_graph<-girafe(ggobj = graph2_reworked)

# ggigraph did not use fig.alt text set above, this is workaround.
browsable(
  tags$div(
    `aria-label` = "This graph is an interactive Boxplot graph with one boxplot for each year showing the mpg of all cars. This Boxplot shows that the average remained around 19 mpg until 2008. From 2009 it started to increase reaching 23.7 in 2017. Even in 1984 outliers with mpg above 30 existed.",
    role = "img",
    interactive_graph
  )
)
...
saveWidget(widget=partial_bundle(interactive_graph), file = "../figures/project-1-graph-2-interactive-graph-.html", selfcontained = TRUE)
```
...
```{r, fig.alt="This graph consists of a scatterplot of the top 10% of cars by MPG. The trend  line starts at 30mpg in 1984 decreases a little bit until 2006 to then increase to about 35mpg by 2017.", warning=FALSE, message=FALSE}
...
```{r, warning=FALSE, message=FALSE, fig.alt="Graph consisting of for scatterplots with each showing the MPG trend for a fuel type. Premium and regular start at about 20 mpg remain stable until 2008 and then increase to 24.95 for regular and 21.69 for Premium by 2017. Diesel starts at 24.82 then decreases to 16.61 in 1994 then increases to 35.62 by 2002 to then decrease to 20.57 by 2008 to then increase to 21.69 by 2017. Electric cars start at 16.72 MPGe (adjusted) in 1998 and increase to 39.93 by 2017."}
...
```{r, warning=FALSE, message=FALSE, fig.alt="A line chart displaying the amount of cars by each fuel type for each year. Most cars are Regular starting at 1702 in 1984 but decreased to 559 by 2017. Premium cars started at 101 in 1985 and increased to 541 by 2017. Diesel starts at 262 in 1984 decreses sharply ending at 15 in 2017"}
